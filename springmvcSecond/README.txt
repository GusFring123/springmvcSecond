这是使用了非注解的方式实现的springmvc的初步配置
	...
	2.所以说需要配置一下handler（controller）	
		...
		在mapping中配置props，在props中配置prop，在prop中配置key与value（可以有多个），key是url地址，value是handler的id，这样就可以形成映射了。
	...
	执行的结果是返回一个ModelAndView，交给中央控制器DispatcherServlet来进行调度，将其交给试图解析器InternalResourceViewResolver进行视图解析，完成之后回来再交给DispatcherServlet转发交给？？？进行渲染后在返回一个view之后再转发给浏览器进行展示。	
